...
def sentiment_classifier_basic(sentence, g,polarity_dic,pol_val_dic,swn_positive_dic,swn_negative_dic):
    ...
    for token in sentence:
        if token in pol_val_lst:
            pol_val = pol_val_dic.get(token)
            swn_positive = swn_positive_dic.get(token)
            swn_negative = swn_negative_dic.get(token)
            sentiment_score = polarity_dic.get(token)

            pol_val_lst.append(pol_val)
            swn_positive_lst.append(swn_positive)
            swn_negative_lst.append(swn_negative)
            ...
            for row in qres:
                ...
                pol_val_dic[token] = pol_val
                swn_positive_dic[token] = swn_positive
                swn_negative_dic[token] = swn_negative
    ...
    return [sentiment_score_final, pol_val_lst, swn_positive_lst, swn_negative_lst]
...
def main():
    ...
    with open("pol_val_cv_documents.txt", "rb") as input_file:
        pol_val_documments = pickle.load(input_file)
    
    with open("swn_positive_cv_documents.txt", "rb") as input_file:
        swn_positive_cv_documments = pickle.load(input_file)

    with open("swn_negative_cv_documents.txt", "rb") as input_file:
        swn_negative_cv_documments = pickle.load(input_file)

    with open("sentiment_score_cv_documents.txt", "rb") as input_file:
        sentiment_score_cv_documments = pickle.load(input_file)
    '''
    



    #pre_processed_documments = pre_processed_documments[0:200]
    ...
    '''
    pol_val_documents_lst = []
    swn_positive_documents_lst = []
    swn_negative_documents_lst = []
    ...
    for review in pre_processed_documments:
        ...
        pol_val_document_lst = []
        swn_positive_document_lst = []
        swn_negative_document_lst = []

        for sentence in review:
            #Get sentiment class of sentence
            [sentiment_score_final,pol_val_lst, swn_positive_lst, swn_negative_lst]  =  sentiment_classifier_basic(sentence,g,polarity_dic,pol_val_dic,swn_positive_dic,swn_negative_dic)
            
            sentiment_score_sentences.append(sentiment_score_final)
            pol_val_document_lst = pol_val_document_lst + pol_val_lst 
            swn_positive_document_lst =  swn_positive_document_lst + swn_positive_lst
            swn_negative_document_lst =  swn_negative_document_lst + swn_negative_lst
            
            
        sentiment_score_reviews.append(sentiment_score_sentences)

        pol_val_documents_lst.append(pol_val_document_lst)
        swn_positive_documents_lst.append(swn_positive_document_lst)
        swn_negative_documents_lst.append(swn_negative_document_lst)
    ...
    with open("pol_val_cv_documents.txt", "wb") as fp:   #Pickling
        pickle.dump(pol_val_documents_lst, fp)

    with open("swn_positive_cv_documents.txt","wb") as fp:
        pickle.dump(swn_positive_documents_lst,fp)

    with open("swn_negative_cv_documents.txt","wb") as fp:
        pickle.dump(swn_negative_documents_lst,fp)
    
    with open("sentiment_score_cv_documents.txt","wb") as fp:
        pickle.dump(sentiment_score_reviews,fp)
